...
import { ERC20ExtendedUpgradeable } from "../../src/ERC20ExtendedUpgradeable.sol";

import { IERC20ExtendedHarness } from "./IERC20ExtendedHarness.sol";

contract ERC20ExtendedUpgradeableHarness is IERC20ExtendedHarness, ERC20ExtendedUpgradeable {
    ...
    function initialize(string memory name_, string memory symbol_, uint8 decimals_) external initializer {
        __ERC20ExtendedUpgradeable_init(name_, symbol_, decimals_);
    }
    ...
    function setAuthorizationState(address authorizer_, bytes32 nonce_, bool isNonceUsed_) external {
        _getERC3009StorageLocation().authorizationState[authorizer_][nonce_] = isNonceUsed_;
    }
